...
void Apskaiciuoti_Rezultatus(std::vector<studentas>& S, std::vector<studentas>& N)
{
	...
	// Apibrëþiamas skirstymo sàlygos lambda iðraiðka
	auto partitionCondition = [&](const studentas& s) {
		return RusiavimoPasirinkimas == 'V' ? s.GalutinisV < 5 : s.GalutinisM < 5;
		};

	// Studentai skirstomi pagal sàlygà
	auto partitionPoint = std::partition(S.begin(), S.end(), partitionCondition);

	// Studentai, atitinkantys sàlygà, perkelti á vektoriø N
	std::vector<studentas> N_temp;
	std::move(partitionPoint, S.end(), std::back_inserter(N_temp));
	N.swap(N_temp);

	// Studentai paðalinami ið vektoriaus S
