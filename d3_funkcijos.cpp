...
void Apskaiciuoti_Rezultatus(std::deque<studentas>& S, std::deque<studentas>& N)
{
	...
	auto isFailCondition = [&](const studentas& studentas) {
		return (RusiavimoPasirinkimas == 'V' && studentas.GalutinisV < 5) || (RusiavimoPasirinkimas == 'M' && studentas.GalutinisM < 5);
		};

	auto it = std::partition(S.begin(), S.end(), isFailCondition);

	for (auto it_fail = it; it_fail != S.end(); ++it_fail)
	{
		N.push_back(std::move(*it_fail));
	}
...
void Spausdinti_Rezultatus(const std::deque<studentas>& S, const std::deque<studentas>& N)
{
	...
	for (int i = 0; i < S.size(); i++)
	{
		...
		Galvociu_failas << std::setw(7) << i + 1 << std::setw(20) << S[i].pavarde << std::setw(20) << S[i].vardas << std::setw(20) << std::setprecision(3) << S[i].GalutinisV << std::setw(20) << std::setprecision(3) << S[i].GalutinisM << std::endl;
